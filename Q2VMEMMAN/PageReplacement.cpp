...
int fifoReplacement(const vector<int>& pages, int frames) {
    vector<int> memory; //This is a representation of the frames list
    int faults = 0; //used to track page faults
...
int lruReplacement(const vector<int>& pages, int frames) { 
    vector<int> memory; //This is a representation of the frames list
    int faults = 0; //used to track page faults
...
int mruReplacement(const vector<int>& pages, int frames) {
    vector<int> memory; //This is a representation of the frames list
    int faults = 0; //used to track page faults
...
int optimalReplacement(const vector<int>& pages, int numFrames) {
    vector<int> memory; //This is a representation of the frames list
    int faults = 0;
...
// Print results
void print(int pageSize, int numPages, int frames, const vector<int>& faults) {
    cout << setw(10) << "Page Size" << setw(15) << "# Pages" << setw(15) << "# Frames"
         << setw(15) << "Algorithm" << setw(20) << "Faults" << endl;
...
void runAlgorithms(vector<int>& data) {
    int frames = 8; //sets the number of memory frames 
    int pageSize = 512; //sets the system page size
    vector<int> faults(4,0); //Initialize the vector to be size 4 (one for each algorithm) and default values of zero
    ...
    faults[0] = fifoReplacement(pages, frames); //returns how many page faults will occur from FIFO page replacement
    faults[1] = lruReplacement(pages, frames);//returns how many page faults will occur from LRU page replacement
    faults[2] = mruReplacement(pages, frames);//returns how many page faults will occur from MRU page replacement
    faults[3] = optimalReplacement(pages, frames);//returns how many page faults will occur from optimal page replacement
